...
export default function LogsClient({
  ...
  useEffect(() => {
    async function loadWallets() {
      const res = await getAgentWalletsWithBalances();
  ...
  useEffect(() => {
    ...
    async function fetchFunds() {
      setIsLoading(true);
      const res = await getAgentFundRequestsPaginated({
        page: fundPage,
        limit: 20,
  ...
  }, [fundPage, startDate, endDate, selectedStatus, selectedWallets, fundRefresh]);
  ...
  useEffect(() => {
    ...
    async function fetchExpenses() {
      setIsLoading(true);
      const res = await getAgentDailyExpensesPaginated({
        page: expensePage,
        limit: 20,
        startDate: startDate || undefined,
        endDate: endDate || undefined,
  ...
  }, [expensePage, startDate, endDate, expenseRefresh]);
  ...
  useEffect(() => {
    ...
    async function fetchEdits() {
      setIsLoading(true);
      const res = await getAgentEditRequestsPaginated({
        page: editPage,
        limit: 20,
        startDate: startDate || undefined,
        endDate: endDate || undefined,
  ...
  }, [editPage, startDate, endDate, selectedStatus, editRefresh]);
  ...
  return (
    <div className="space-y-6 text-right font-cairo select-none" dir="rtl">
      ...
      <div className="flex flex-col gap-1 text-right">
        <h1 className="text-2xl md:text-3xl font-extrabold text-white">سجل العمليات</h1>
        <p className="text-sm text-brand-dim">تابع حالة طلباتك المالية وتقارير مصاريفك اليومية السابقة بكل شفافية وسهولة.</p>
      </div>
      ...
      <div className="bg-[#0c0f1d] border border-brand-border/40 p-4 rounded-2xl flex flex-wrap items-end gap-5 w-full">
        ...
        {activeTab !== "daily-expenses" && (
          <div className="space-y-1 w-full sm:w-[210px] relative" ref={statusDropdownRef}>
        ...
        {activeTab === "fund-requests" && (
          <div className="space-y-1 w-full sm:w-[220px] relative" ref={walletDropdownRef}>
            <label className="text-[10px] font-bold text-brand-dim/60 block pr-1 font-cairo">المحفظة المستلمة</label>
            <button
              type="button"
              dir="rtl"
              onClick={() => {
            ...
            >
              <span className={`text-white text-xs font-cairo ${selectedWallets.length === 1 ? "font-mono" : ""}`}>
                {selectedWallets.length === 0
                  ...
                  ? "محفظتين"
                  : selectedWallets.length >= 3 && selectedWallets.length <= 10
                  ? `${selectedWallets.length.toLocaleString("en-US")} محافظ`
                  : `${selectedWallets.length.toLocaleString("en-US")} محفظة`}
              </span>
        ...
        {(startDate || endDate || selectedStatus !== "all" || selectedWallets.length > 0) && (
          <button
            onClick={handleResetFilters}
      ...
      <div className="bg-[#0a0d16]/60 border border-brand-border/40 backdrop-blur-xl rounded-3xl p-6 shadow-xl animate-fade-in min-h-[350px] relative">
        ...
        {activeTab === "fund-requests" && (
          <div className="space-y-4">
            <h3 className="text-base font-bold text-white mb-2">تاريخ طلبات شحن الرصيد والعهدة</h3>
                ...
                <table className="w-full text-right border-collapse min-w-[800px]">
                  <thead>
                    <tr className="border-b border-brand-border/40 text-brand-dim/70 text-xs font-bold">
                      <th className="pb-3.5 pr-2 font-bold">التاريخ</th>
        ...
        {activeTab === "daily-expenses" && (
          <div className="space-y-4">
            <h3 className="text-base font-bold text-white mb-2">سجل المصاريف والتحويلات وإغلاق اليوم</h3>
            ...
            {dailyExpenses.length === 0 ? (
              ...
              <div className="overflow-x-auto custom-scrollbar">
                <table className="w-full text-right border-collapse min-w-[700px]">
                  <thead>
                    <tr className="border-b border-brand-border/40 text-brand-dim/70 text-xs font-bold">
                      <th className="pb-3.5 pr-2 font-bold">تاريخ التقرير</th>
                      <th className="pb-3.5 font-bold">الإجمالي اليومي</th>
                      <th className="pb-3.5 font-bold">تسويق 1 (CPL)</th>
                      <th className="pb-3.5 font-bold">تسويق 2 (CPL)</th>
                      <th className="pb-3.5 font-bold">تسويق 3 (CPL)</th>
        ...
        {/* TAB 3: Edit Expense Requests */}
        {activeTab === "edit-requests" && (
          <div className="space-y-4">
            <h3 className="text-base font-bold text-white mb-2">طلبات تعديل تقارير المصاريف السابقة</h3>
            ...
            {editRequests.length === 0 ? (
              ...
              <div className="overflow-x-auto custom-scrollbar">
                <table className="w-full text-right border-collapse min-w-[750px]">
                  <thead>
                    <tr className="border-b border-brand-border/40 text-brand-dim/70 text-xs font-bold">
                      <th className="pb-3.5 pr-2 font-bold">تاريخ الطلب</th>
                      <th className="pb-3.5 font-bold">تاريخ تقرير المصاريف</th>
                      <th className="pb-3.5 font-bold">التعديلات المطلوبة</th>
                      <th className="pb-3.5 font-bold text-center">حالة الطلب</th>
                      <th className="pb-3.5 pl-2 font-bold text-left">توقيت الإجراء</th>
                    </tr>
                  </thead>
